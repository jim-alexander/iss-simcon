...
export default class Timesheets extends Component {
  ...
  loadTimesheet() {
    ...
    let isAllowed = allowed.findIndex(id => id === this.props.user.id)
    if (isAllowed !== -1) {
      this.props.dailyPrestarts.forEach(prestart => {
        if (
          (prestart.form_values['80e9'] > this.state.start && prestart.form_values['80e9'] < this.state.end) ||
          (prestart.form_values['80e9'] === this.state.start || prestart.form_values['80e9'] === this.state.end)
        ) {
          if (prestart.form_values['86b7']) {
            prestart.form_values['86b7'].forEach(entry => {
              if (entry.form_values['cc82'] === 'company_personnel') {
                ...
                var overTimeOne =
                  this.calcOverTimeOne(start, end, moment(prestart.form_values['80e9']).format('dddd')) !== 0
                    ...
                    : 0
                var overTimeTwo =
                  this.calcOverTimeTwo(start, end, moment(prestart.form_values['80e9']).format('dddd')) !== 0
                ...
                var absent = entry.form_values['c9b8']
                  ? entry.form_values['c9b8'].choice_values[0] !== 'No'
                    ? entry.form_values['c9b8'].choice_values[0]
                    : null
                  : null

                const index = data.findIndex(e => e.name === obj.name)
                if (index === -1) {
                  obj.record_id = prestart.id
                  obj[moment(prestart.form_values['80e9']).format('D-MMM')] = absent ? absent : null
                  obj.ot1 = overTimeOne
                  obj.ot2 = overTimeTwo
                  obj.travel = travel
                  obj.lafha = lafha
                  ...
                  if (data[index][moment(prestart.form_values['80e9']).format('D-MMM')]) {
                    ...
                    data[index].hours_minus = data[index].hours_minus
                      ...
                      .add(parseInt(addMins, 0), 'minutes')
                      .subtract(
                        overTimeOneMultiple(data[index].hours, moment(prestart.form_values['80e9']).format('dddd')),
                        'hours'
                      )
                      .subtract(
                        overTimeTwoMultiple(data[index].hours, moment(prestart.form_values['80e9']).format('dddd')),
                        'hours'
                      )
    ...
    return (
      <div>
        <div>
          ...
          <Row gutter={10}>
            <Col xs={24} sm={24} md={10} lg={10} xl={10} style={{ marginBottom: '10px' }}>
              {this.selectDate()}
            </Col>
            <Col xs={24} sm={24} md={10} lg={10} xl={10} style={{ marginBottom: '10px' }}>
              {this.dateTo()}
            </Col>
            <Col xs={24} sm={24} md={4} lg={4} xl={4} style={{ marginBottom: '10px' }}>
