...
//Field Checkers
import { safetyWalkChecker } from './safetyWalkCheckerBackup';
import { taskObservationChecker } from './taskObservationChecker';
import { siteInspectionChecker } from './siteInspectionChecker';
...
export default class HazardRegister extends Component {
  ...
  loadHazardData() {
    ...
    const createObj = (hazard, recordedBy, assignedTo, dateIdentified, description, closeOutDate, formValues, closeOutLocation) => {
      let status = hazard.status === 'Closed Out' || closeOutDate ? 'Closed Out' : 'Action Required';
      const job = () => {
    ...
    if (this.state.selectedJob.length === 0) {
      ...
      this.props.safetyWalk.forEach(inspection => {
        ...
        safetyWalkChecker(inspection.form_values).forEach(entry => {
          if (entry.comments) {
            data.push(
              createObj(
                inspection,
      ...
      this.props.siteInspection.forEach(inspection => {
        let inspectedBy = inspection.form_values['0923'] ? inspection.form_values['0923'].choice_values[0] : null;
        let dateInspected = inspection.form_values['91dd'];
        siteInspectionChecker(inspection.form_values).forEach(entry => {
          if (entry.comments) {
            data.push(
              createObj(
                inspection,
      ...
      this.props.taskObservation.forEach(inspection => {
        let inspectedBy = inspection.form_values['0923'] ? inspection.form_values['0923'].choice_values[0] : null;
        let dateInspected = inspection.form_values['cc7d'];
        taskObservationChecker(inspection.form_values).forEach(entry => {
          if (entry.comments) {
            data.push(
              createObj(
                inspection,
  ...
  closeHazard(hazard) {
    ...
    //The hazard being closed out is from the hazard register form on fulcrum
