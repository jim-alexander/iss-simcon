...
export default class HazardRegister extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      prevProps.hazards !== this.props.hazards
    ) {
      db.lastViewedPage(this.props.user.id, "Hazard Register");
      this.loadHazardData();
    }
  ...
  loadHazardData() {
    ...
    const createObj = (
      ...
      let status =
        hazard.status === "Closed Out" || closeOutDate
          ? "Closed Out"
          : "Action Required";
      const job = () => {
        let index = this.props.jobFiles.findIndex(
          job => job.project_id === hazard.project_id
        );
        let jobNumber =
          index > 0 ? this.props.jobFiles[index].form_values["5f36"] : "none";
        return jobNumber;
      };
      let obj = {
    ...
    if (this.state.selectedJob.length === 0) {
      this.props.hazards.forEach(hazard => {
        let recordedBy = hazard.form_values["4a01"]
          ? hazard.form_values["4a01"].choice_values[0]
          : "";
        let assignedTo = hazard.form_values["81c2"]
          ? hazard.form_values["81c2"].choice_values[0]
          : "";
        data.push(
          createObj(
            hazard,
      ...
      this.props.toolboxMinutes.forEach(toolbox => {
        if (toolbox.form_values["042c"]) {
          let recordedBy = toolbox.form_values["014b"]
            ? toolbox.form_values["014b"].choice_values[0]
            : "";
          let assignedTo = toolbox.form_values["6718"]
            ? toolbox.form_values["6718"].choice_values[0]
            : "";
          data.push(
            createObj(
              toolbox,
      ...
      this.props.dailyDiarys.forEach(diary => {
        if (diary.form_values["eda0"]) {
          let recordedBy = diary.form_values["cfa2"]
            ? diary.form_values["cfa2"].choice_values[0]
            : "";
          let assignedTo = diary.form_values["5b46"]
            ? diary.form_values["5b46"].choice_values[0]
            : "";
          data.push(
            createObj(
              diary,
      ...
      this.props.siteInspections.forEach(inspection => {
        let inspectedBy = inspection.form_values["0923"]
          ? inspection.form_values["0923"].choice_values[0]
          : null;
  ...
  closeHazard(hazard) {
    ...
    if (hazard.formValues.form_id === "3e7888a5-26fa-449d-a183-b5a228c6e59a") {
      ...
      update.form_values[hazard.closeOutLocation] = moment().format(
        "YYYY-MM-DD"
      );
