  ...
  componentDidMount() {
    if (firebase.auth.currentUser) {
      db.getCurrentUsername(firebase.auth.currentUser.uid)
        ...
        .catch(err =>
          message.error(
            "There has been an error loading user data. Please be patient. Error: " +
              err,
            10
          )
        );
    ...
    if (
      ...
      this.setState({
        ...
        plantVerifications: JSON.parse(
          localStorage.getItem("plantVerifications")
        ),
    ...
    Promise.all(promises)
      ...
      .then(() => {
        ...
        localStorage.setItem(
          "jobFiles",
          JSON.stringify(saveRecentData(this.state.jobFiles, 10))
        );
        localStorage.setItem(
          "dailyPrestarts",
          JSON.stringify(saveRecentData(this.state.dailyPrestarts, 150))
        );
        localStorage.setItem(
          "plantVerifications",
          JSON.stringify(saveRecentData(this.state.plantVerifications, 20))
        );
        localStorage.setItem(
          "siteInspections",
          JSON.stringify(saveRecentData(this.state.siteInspections, 20))
        );
        localStorage.setItem(
          "toolboxMinutes",
          JSON.stringify(saveRecentData(this.state.toolboxMinutes, 10))
        );
        localStorage.setItem(
          "dailyDiarys",
          JSON.stringify(saveRecentData(this.state.dailyDiarys, 50))
        );
        localStorage.setItem(
          "hazards",
          JSON.stringify(saveRecentData(this.state.hazards, 100))
        );
        localStorage.setItem(
          "incidentNonConf",
          JSON.stringify(saveRecentData(this.state.incidentNonConf, 100))
        );
    ...
    return (
      <Layout>
        ...
        <Layout className="layoutContent">
          <Online>
            <Tooltip
              title="Data loads automatically after 10 minutes."
              mouseEnterDelay={2}
              placement="bottom">
              <div
                id="lastLoaded"
                onClick={() => this.loadFulcrumData("Button Refresh")}
                className="printHide">
                <span id="lastLoadedDefault">
                  Data Last Loaded {this.state.lastLoaded}
                </span>
          ...
          <Content style={{ margin: "24px 16px 0", minHeight: "89vh" }}>
              ...
              <Route
                ...
                render={props => (
                  <Job
                    {...props}
                    user={this.state.user}
                    devicesLastSynced={this.state.devicesLastSynced}
                  />
                )}
              />
            </div>
            <Button
              block
              onClick={this.PageNotes}
              style={{ maxWidth: 200, margin: "15px auto" }}
              className="printHide">
              Page Notes
            </Button>
          </Content>
